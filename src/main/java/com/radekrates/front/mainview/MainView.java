...
public class MainView extends VerticalLayout {
    ...
    private Label instructionIban = new Label("Please, provide the Iban number as shown in following example: EExxxxxxxxxxxxxxxxxx " +
            "where E - letter (country code), x - numeric case. x quantity is 18.");
    ...
    public MainView(@Autowired DataTransfer dataTransfer) {
        ...
        VerticalLayout mainContentIban = new VerticalLayout(ibanDtoGrid, ibanForm);
        mainContentIban.setSizeFull();


        add(logo, logOut, filterCurrencyCode, filterIbanNumber, instructionIban, mainContentIban);
        ...
        add(filterPairTransaction, instuctionTransaction, mainContentTransaction);
        //setSizeFull();
    }
    ...
    private void updateCurrencyCode(DataTransfer dataTransfer) {
        ibanDtoGrid.setItems(dataTransfer.getIbans(dataTransfer.getUserToMain(), "currencyCode", filterCurrencyCode.getValue()));
    }

    private void updateIbanNumber(DataTransfer dataTransfer) {
        ibanDtoGrid.setItems(dataTransfer.getIbans(dataTransfer.getUserToMain(), "ibanNumber", filterIbanNumber.getValue()));
    }

    public void updateTransactionPair(DataTransfer dataTransfer) {
        transactionGrid.setItems(dataTransfer.getTransactions(dataTransfer.getUserToMain(), "pair", filterPairTransaction.getValue()));
    }
}
